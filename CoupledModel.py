...
class CoupledHH():
    def __init__(self, k01=0.10, k10=0.10, gbar_K=36, gbar_Na=120, gbar_L=0.3, V_K=-12, V_Na=115, V_L=10.613, C_m=1.0, dt=0.01):
    ...
    def derivatives_fb(self, y0, t):
        """ The differential equations for a feedback loop """
        V0, n0, m0, h0, V1, n1, m1, h1 = y0

        dVdt0 = (self.I_in(t) - (self.I_K(V0, n0) + self.I_Na(V0, m0, h0) + self.I_L(V0)) - self.k10*(V1 - V0)) / self.C_m
        dndt0 = self.alpha_n(V0) * (1 - n0) - self.beta_n(V0) * n0
        dmdt0 = self.alpha_m(V0) * (1 - m0) - self.beta_m(V0) * m0
        dhdt0 = self.alpha_h(V0) * (1 - h0) - self.beta_h(V0) * h0
        
        dVdt1 = (- (self.I_K(V1, n1) + self.I_Na(V1, m1, h1) + self.I_L(V1)) - self.k01*(V0 - V1)) / self.C_m
        dndt1 = self.alpha_n(V1) * (1 - n1) - self.beta_n(V1) * n1
        dmdt1 = self.alpha_m(V1) * (1 - m1) - self.beta_m(V1) * m1
        dhdt1 = self.alpha_h(V1) * (1 - h1) - self.beta_h(V1) * h1

        return dVdt0, dndt0, dmdt0, dhdt0, dVdt1, dndt1, dmdt1, dhdt1

    def run(self, y0, t, I_in = None, method="feedforward", plot=False):
        """ Returns a matrix of I_inj, V, n, m and h at each timestep
        param y0: array of starting values [V0, n0, m0, h0, V1, n1, m1, h1,] or [V0, V1]
        ...
        if (len(y0) == 2):
            V0 = y0[0]
            V1 = y0[1]
            n0, m0, h0 = self.set_init_conditions(V0)
            n1, m1, h1 = self.set_init_conditions(V1)
            y0 = [V0, n0, m0, h0, V1, n1, m1, h1]
        
        if method == "feedback":
            sol = odeint(self.derivatives_fb, y0, self.t_points)
        else:
            # only feedforward
            self.k10 = 0
            sol = odeint(self.derivatives_fb, y0, self.t_points)
        ...
        if plot:
            ...
            ax2.plot(self.t_points, sol[:, 0], color="red", linewidth=1, label="V1")
            ax2.set_ylabel("Voltage (mV)")
            ax2.set_xlabel("Time (ms)")
            
            ax2.plot(self.t_points, sol[:, 4], color="green", linewidth=1, label="V2")
            ax2.legend(loc="best")
